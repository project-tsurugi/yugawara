...
using expression_ref = ::takatori::util::ownership_reference<scalar::expression>;
...
std::unique_ptr<scalar::expression> search_key_term::purge_equivalent_factor() {
    return purge(equivalent_term_, equivalent_factor_);
}

std::unique_ptr<scalar::expression> search_key_term::clone_equivalent_factor() {
    return clone_unique(equivalent_factor_->value());
}

std::unique_ptr<scalar::expression> search_key_term::purge_lower_factor() {
    return purge(lower_term_, lower_factor_);
}

std::unique_ptr<scalar::expression> search_key_term::purge_upper_factor() {
    return purge(upper_term_, upper_factor_);
}

std::unique_ptr<scalar::expression> search_key_term::purge(
        std::optional<expression_ref>& term,
        std::optional<expression_ref>& factor) {
    if (term) {
        auto result = factor->exchange({});
        *term = make_boolean_expression(true);
        return result;
    }
